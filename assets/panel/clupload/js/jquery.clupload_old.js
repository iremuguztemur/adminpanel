...
$.fn.clupload = function(func) {
    ...
    this.initialize = function() {
		...
		content.click(function(event) {
			if (event.target.id == content.attr('id')) {
				$('#clfile').click();
			}			
		});

		/* DRAG AND DROP HTML5 */
		var holder = document.getElementById($(this).attr('id'));
		...
		holder.ondrop = function (event) {
			...
			if (event.dataTransfer.files.length > 0) {
				window.dropbox = event.dataTransfer.files;
				addPhoto(0);
			}			
		}
		/* DRAG AND DROP HTML5 */
	...
	function addPhoto(fid) {
		if (window.dropbox) {
			var dosyaarray = window.dropbox;
		} else {
			var dosyaarray = $(content.options.target).get(0).files;
		}

		err = false;
		totalSize = 0;
		$.each(dosyaarray, function(index, val) {
			if (val.type == 'image/jpeg' || val.type == 'image/png' || val.type == 'image/bmp') {
				totalSize = totalSize+val.size;			
			} else {
		...
		if (err) {content.options.error.call(content,err);	return false } // any error call error function
	...
	function clAddPhoto(fid) {
		...
		var arg = dosyaarray[fid];	
		var file = arg;
		var nid = fid+1;
		var total = dosyaarray.length;
		info('Image Processing ['+nid+' / '+total+']');

		if (!file) {
			info();$(content).find('.cl-added').remove();
			return false;
		}
		...
		reader.onload = function (e) {
			...
			if (tratio >= ratio) { // crop width
				...
				lastid = content.find('figure:last-child').attr('id');
				var lastid = lastid.split("_")[1]; 
				var lastid = Number(lastid);

    			var tImg = content.find('#gl_0').clone().appendTo(content).removeClass('hidden').show().addClass('cl-added').attr('id','gl_'+Number(lastid+1)).find('img');
				...
				if (dosyaarray[fid+1]) {
					...
					window.drepeat = 5; // failed count renew
					dragUpdate(); 
					info();
				}
			}
	...
	function dragUpdate() {
		...
		content.find('.cl-added').each(function(){
			...
			var cwd = $(this).find('.cl-top').outerWidth();
			var chg = $(this).find('.cl-top').outerHeight();

			var ox = $(this).find('.cl-top').offset().left;
    		var oy = $(this).find('.cl-top').offset().top;

			$(this).find('img').draggable({
				containment: [-1*(iwd-cwd)+ox,-1*(ihg-chg)+oy,ox,oy],
				axis : axis
			});
			
			if (window.drepeat) {
				setTimeout(function(){ dragUpdate() }, window.drepeat*120);
				window.drepeat = window.drepeat-1;
			}
		});
	}
	...
	function clSerialize (id,callback) {
		...
		uImg.onload = function () {
			...
			$('#gl_'+id).attr('data-size',getSizeBase64(SdataUrl));

			content.serializeArray.push({data:SdataUrl,name:name});

			if (id+1 <= figLength) {
				clSerialize(id+1,callback);
			} else {
				info();
	...
	function clSendAjax (id) {  
		var imgLength = content.find('figure').length-1;
		var thisUpImg = $('#gl_'+id).find('img');
		var src = thisUpImg.attr('src');
		uImg = new Image();
		uImg.src = src;
		var uRadius = uImg.naturalWidth / thisUpImg.width();

		var uImgLeft = thisUpImg.css('left').split("px")[0]; 
		var uImgLeft = Math.abs(uImgLeft*uRadius); 

		var uImgTop = thisUpImg.css('top').split("px")[0]; 
		var uImgTop = Math.abs(uImgTop*uRadius); 
				...
				xhr: function() {
					...
					xhr.upload.addEventListener("progress", function(evt){
						if (evt.lengthComputable) {
							...
							info('Uploading Images ['+id+'/'+imgLength+'][%'+Math.round(percentComplete*100)+']')
						}
					}, false);
					return xhr;
				},
